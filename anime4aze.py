...
def get_episodes_api(s_id):
    result = get_episodes(s_id)
    return jsonify(result)

@app.route("/get-episode-dl/<string:s_id>", methods=["GET"])
def get_episode_dl_api(s_id):
    result = get_episode_dl(s_id)
    return jsonify(result)

@app.route("/latest-episodes", methods=["GET"])
def latest_episodes_api():
    result = latest_episodes()
    return jsonify(result)

@app.route("/fetch-seasonals", methods=["GET"])
def fetch_seasonals_api():
    result = fetch_seasonals()
    return jsonify(result)

@app.route("/get-anime-info/<string:s_id>", methods=["GET"])
def get_anime_info_api(s_id):
    result = get_anime_info(s_id)
    return jsonify(result)
